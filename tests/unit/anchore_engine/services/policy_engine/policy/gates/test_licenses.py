...
def test_full_match_trigger(licenses_gate, exec_context, image, test_context):
    ...
    if test_context["expected_fire"]:
        assert full_match_trigger.did_fire
        assert len(full_match_trigger.fired) == 1
        assert full_match_trigger.fired[0].msg == test_context["expected_msg"]
    else:
        assert not full_match_trigger.did_fire
...
def test_substring_match_trigger(licenses_gate, exec_context, image, test_context):
    ...
    if test_context["expected_fire"]:
        assert substring_match_trigger.did_fire
        assert len(substring_match_trigger.fired) == 1
        assert substring_match_trigger.fired[0].msg == test_context["expected_msg"]
    else:
        assert not substring_match_trigger.did_fire
